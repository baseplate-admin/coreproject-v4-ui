...
>
	<div class="modal-box mx-auto flex max-h-max w-max flex-col items-center bg-secondary md:p-[2vw]">
								...
								<div bind:this={anime_search_results_container_element}>
									{#each results as item, index}
										...
										{@const mapping = [
											...
											{
												value: item.aired_from
													? new FormatDate(item.aired_from).format_to_human_readable_form
													: null,
												class: "flex items-center gap-[0.3vw]"
											}
										]}
										<a
											...
											class={cn(
												is_active && "bg-neutral/50",
												"flex w-full auto-rows-max items-center gap-[1vw] rounded-[0.7vw] p-[0.8vw] transition duration-200 hover:bg-neutral/75"
											)}
										>
											<img
												src={item.cover}
											...
											<div class="flex flex-col leading-none md:gap-[0.5vw]">
												<span
													class="line-clamp-1 text-[1.1vw] font-semibold leading-none text-white"
												>
													{item.name}
													<span class="font-medium text-accent/50">({item.name_japanese})</span>
												</span>
							...
							{:else}
								...
								>
									...
									<span class="text-center font-semibold leading-none text-error"
										>Couldn't find animes with: "{search_query}"</span
									>
								</div>
							{/if}
						{:catch error}
		...
		<p class="mt-[2vw] text-[0.8vw]">
			...
			<kbd class="kbd kbd-xs rounded-full text-[0.8vw]">▲</kbd>
			<kbd class="kbd kbd-xs rounded-full text-[0.8vw]">▼</kbd>
			and
			<kbd class="kbd kbd-xs rounded-full text-[0.8vw]">Tab</kbd>
			keys.
		</p>
	</div>
